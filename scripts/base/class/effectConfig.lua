...
local function getWidth(configObj,key)
    if configObj._propertyValues[key] == nil then
        return Graphics.sprites.effect[configObj.id]:getWidth()
    else
...
local function getHeight(configObj,key)
    if configObj._propertyValues[key] == nil then
        return Graphics.sprites.effect[configObj.id]:getHeight() / (configObj.frames * framestyleEffect[configObj.framestyle]) / configObj.variants
    else
        return configObj._propertyValues[key]
    end
end
...
setmetatable(effectConfig, {__index = function(self, id)
	local path = "config/effect/effect-".. id.. ".txt"
	local resolved = File.resolve(path)
	
	if resolved then
		print(resolved)
		rawset(self, id, convertParsedIntoConfig(id,ini.read(resolved)))
	else
		resolved = File.resolve('effect-' .. id .. '.txt')
