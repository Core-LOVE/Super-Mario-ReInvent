...
Effect.config = require("class/effectConfig")
...
function Effect:render(args)
    ...
    local texture = args.texture or Graphics.sprites.effect[self.id]
...
function Effect.onTickInternal()
	Effect.onPhysics()
end

function Effect.onDrawInternal()
    for i = #Effect.objs, 1, -1 do
        local obj = Effect.objs[i]
...
function Effect.onInit()
	registerEvent(Effect, 'onTickInternal')
	registerEvent(Effect, 'onDrawInternal')
end
